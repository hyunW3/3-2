...
def timeout(target_ID):
	...
	print(target_ID,"is disappear","\t",client_list[target_ID])
...
#	server_socket.setsockopt(SOL_SOCKET,SO_REUSEADDR, 1)
	...
	while True:
		...
		if instr == keep_alive :
			...
			packet += (serverID +"\n\r").encode()
			packet += (str(clientID)+"|"+str(addr)).encode()
			tmp_socket = socket(AF_INET, SOCK_DGRAM)
			for ID,address in client_list.items():
				tmp_socket.sendto(packet,address)
			tmp_socket.close()
			...
			client_time[clientID] = timer
			print("======log in ID======")
			#print(clientID," is ",client_list[clientID])
			...
			print(target_ID,"is deregister","\t",client_list[target_ID])
			del client_list[target_ID]
