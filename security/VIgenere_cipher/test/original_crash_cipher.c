...
//double percent[27] = {18.31, 10.22, 7.51, 6.55, 6.20, 5.7, 5.7, 5.33, 4.97, 4.86, 3.36, 3.35, 2.30, 2.27, 2.02, 1.97, 1.69, 1.64, 1.5, 1.47, 1.27, 0.79, 0.57, 0.15, 0.11, 0.09, 0.06};
double percent[27] = {18.31, 12.60, 9.22, 8.01, 7.56, 7.00, 6.95, 6.52, 6.12, 6.11, 4.07, 4.03, 2.79, 2.74, 2.49, 2.43, 2.09, 1.99, 1.85, 1.75, 1.54, 0.98, 0.69, 0.17, 0.12, 0.10, 0.06};
...
//=======================
    ...
    //unsigned char *key;
    //key = (unsigned char*) malloc(sizeof(unsigned char)*(length_of_key+1));
    ...
    for(int k=0; k<length_of_key; ++k){
        ...
        for(int i=0; i<=0xFF; ++i){
            if(max_cnt.max < counts[i]){
                ...
                max_cnt.nu = i;
                //printf("max :%d(%dth), submax : %d(%dth)\n",max_cnt.max,max_cnt.nu,submax_cnt.max,submax_cnt.nu,' ');
